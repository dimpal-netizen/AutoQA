...
**AutoQA works without an API key.** Recording, generating test cases and
running them all work with nothing configured. This is deliberate: recordings
become Playwright code through ordinary Python, not through a model, so the part
that actually runs never depends on a network call succeeding.

What a key buys is judgement — explaining a failure, writing a bug report,
inventing a case nobody recorded. Those say so plainly when you reach them.
...
| Extra cases built from what you recorded | ✅ | ✅ |
| Readable names (`sign_in_button` vs `button_2`) | ❌ | ✅ |
| Extra cases nobody recorded: security, unrecorded flows | ❌ | ✅ |
| Failure analysis — root cause and fix | ❌ | ✅ |
| Drafted bug reports | ❌ | ✅ |

**Generate test cases works without a key.** With one, a model reads the
recording and invents the cases a thorough tester would add. Without one, the
cases are built from the recording itself — each page rechecked for the elements
you used on it, and each form you filled in resubmitted with one field left
empty, filled with spaces, or given a value longer than its format allows. Those
are narrower than a model's — each case's **Generated by** column says which of
the two wrote it — but they are real tests, and they run on all three browsers.
